        ...
        case 13:  // pulse
          {
            ...
            else  // is split beat, split_begin and split_end are constant for duration of voice node
            {
              if (pulse1->split_end < pulse1->split_begin)  // end always larger for split beat, swap if not
              {
                ...
                pulse1->split_now = (pulse1->split_begin + pulse1->split_end)/2.; // set working split to the new begin
              }
        ...
        case 14:  // pulse step slide, have to create list of steps and slides
          { 
            ...
            else  // both begin and end split are user specified
            {
              ...
              pulse1->split_now = pulse1->split_begin;      // set working split to begin for both cases
            }
            ...
            for (ii = 1; ii < total_nodes; ii++)  // create rest of step list nodes
            {
              ...
              else  // have to take care of pan across nodes here, so that each node starts at end of previous.
              {
                double fraction_begin = ((double) (ii)/(double) total_nodes);  // fraction of split at start of node
                double fraction_end = ((double) (ii+1)/(double) total_nodes);  // fraction of split at end of node
                double split_begin = pulse1->split_begin + ((pulse1->split_end - pulse1->split_begin) * fraction_begin);
                double split_end = pulse1->split_begin + ((pulse1->split_end - pulse1->split_begin) * fraction_end);
                pulse3->split_begin = split_begin;  // starting split for this node
                pulse3->split_end = split_end;      // ending split for this node
            ...
            {
              /* split_begin already set, no need to modify */
              double fraction_end = ((double) (1.)/(double) total_nodes);  // fraction of split at end of node
              pulse1->split_end = pulse1->split_begin + ((pulse1->split_end - pulse1->split_begin) * fraction_end);
